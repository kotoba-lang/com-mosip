(ns mosip.main
  "Kotodama WASM entrypoint for the Mosip clean-room actor (L4) — Clojure port.
  ...
  facts (`mosip.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"mosip.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "mosip-compat" :tier tier :entities entities} 200])
